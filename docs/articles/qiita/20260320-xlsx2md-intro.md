...
- `xlsx2md` の紹介を 1 本目の記事テーマにします
- 主役はアプリそのものに置きます
- 開発体験の話は別記事に分離します

## この記事の価値

- 単なる `Excel -> Markdown` ではなく、AI が扱いやすい構造へ寄せることに意味があります
- 地の文や画像を扱えることに価値があります
- 罫線を手がかりに表を推論できることが、このアプリの強みです
- 図形にも一部対応していますが、現時点では対応範囲はかなり限定的です
- Single-file Web App なので配布や試用のハードルが低いです
- オフライン実行できるため、利用場面を選びにくいです
...
- 掲載先: Qiita
- 公開記事タイトル: Excelブックを生成AI向けMarkdownに変換する `xlsx2md` を作りました
...
また、配布形態は Single-file Web App です。実装の正本は TypeScript で、ビルド時に HTML と JavaScript をまとめ、単一の `xlsx2md.html` として扱えるようにしています。これによって、ブラウザだけで動かせる使い勝手と、ソースコード側の保守性を両立しています。
