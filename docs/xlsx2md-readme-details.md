...
- table detection mode として `balanced / border-priority` を切り替えられる
- 画像、表、地の文、リストを扱う
...
- table detection mode は `balanced / border-priority` を扱う
...
- `balanced`: 既定の表検出モード
- `border-priority`: 罫線のある表候補を優先し、borderless fallback 検知を抑えるモード
...
- `balanced`: 表として検出
- `border-priority`: narrative として扱う
