(() => {
  ...
  function shiftReferenceAddress(addressText: string, rowOffset: number, colOffset: number): string {
  ...
  function parseSimpleFormulaReference(formulaText: string, currentSheetName: string): { sheetName: string; address: string } | null {
  ...
  function buildFormulaResolver(workbook: ParsedWorkbook) {
    ...
    function resolveDefinedNameRange(sheetName: string, name: string): { sheetName: string; start: string; end: string } | null {
      ...
      const rowIndex = Number(resolveScalarFormulaValue(
        args[1],
        sheetName,
        resolveCellValue,
      ...
      const colIndex = args.length === 3
        ? Number(resolveScalarFormulaValue(
          args[2],
          sheetName,
          resolveCellValue,
          (targetSheetName, rangeText) => resolveRangeEntries(targetSheetName, rangeText).numericValues,
  ...
  function tryResolveFormulaExpressionLegacy(
    ...
    const concatResult = tryResolveConcatenationExpression(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const numericFunctionResult = tryResolveNumericFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const datePartFunctionResult = tryResolveDatePartFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const predicateFunctionResult = tryResolvePredicateFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const chooseFunctionResult = tryResolveChooseFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const textFunctionResult = tryResolveTextFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const lookupFunctionResult = tryResolveLookupFunction(
      normalized,
      currentSheetName,
      resolveCellValue
    );
    ...
    const stringFunctionResult = tryResolveStringFunction(
      normalized,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const conditionalAggregateResult = tryResolveConditionalAggregateFunction(
      normalized,
      currentSheetName,
      resolveCellValue
    );
    ...
    const replacedRefs = normalized.replace(
      /(?:'((?:[^']|'')+)'|([A-Za-z0-9_ ]+))!(\$?[A-Z]+\$?\d+)|(\$?[A-Z]+\$?\d+)/g,
      (_full, quotedSheet, plainSheet, qualifiedAddress, localAddress) => {
    ...
    const replaced = replaceNumericDefinedNames(
      replacedRefs,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
    ...
    const replacedFunctions = replaceEmbeddedNumericFunctions(
      replaced,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
  ...
  function tryResolveLogicalFunction(
    ...
    if (functionName === "AND") {
      ...
      const booleans = evaluations as boolean[];
  ...
  function tryResolveLookupFunction(
    ...
    if (matchCall) {
      ...
      if (lookupValue == null || !rangeRef) return null;
      if (args.length === 3) {
    ...
    if (indexCall) {
      ...
      if (!start.row || !start.col || !end.row || !end.col) return null;
      const startRow = Math.min(start.row, end.row);
      const endRow = Math.max(start.row, end.row);
      const startCol = Math.min(start.col, end.col);
      const endCol = Math.max(start.col, end.col);
    ...
    if (hlookupCall) {
      ...
      if (lookupValue == null || !rangeRef || Number.isNaN(rowIndex) || rowIndex < 1) return null;
      if (args.length === 4) {
        const rangeLookup = resolveScalarFormulaValue(args[3], currentSheetName, resolveCellValue);
        if (rangeLookup == null) return null;
      ...
      const endCol = Math.max(start.col, end.col);
      const targetRow = startRow + Math.trunc(rowIndex) - 1;
      if (targetRow > endRow) return null;
    ...
    if (lookupValue == null || !rangeRef || Number.isNaN(columnIndex)) return null;
    ...
    if (targetCol > endCol) return null;
  ...
  function tryResolveDatePartFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    if (!parts) return null;
    if (fnName === "YEAR") return String(Number(parts.yyyy));
    if (fnName === "MONTH") return String(Number(parts.mm));
    if (fnName === "DAY") return String(Number(parts.dd));
    if (fnName === "WEEKDAY") {
  ...
  function tryResolvePredicateFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
  ...
  function tryResolveChooseFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    const args = splitFormulaArguments(call.argsText.trim());
    if (args.length < 2) return null;
    const indexValue = resolveNumericFormulaArgument(
      args[0],
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
  ...
  function tryResolveConcatenationExpression(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    for (const segment of segments) {
      const resolved = resolveScalarFormulaValue(
        segment,
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
  ...
  function tryResolveComparisonExpression(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
  ...
  function splitComparisonExpression(expression: string): { left: string; operator: string; right: string } | null {
    const operators = ["<=", ">=", "<>", "=", ">", "<"];
    let depth = 0;
  ...
  function findTopLevelOperatorIndex(expression: string, operator: string): number {
  ...
  function splitConcatenationExpression(expression: string): string[] | null {
    const parts: string[] = [];
    let start = 0;
    let depth = 0;
    let inSingleQuote = false;
  ...
  function parseWholeFunctionCall(
    expression: string,
    allowedNames: string[]
  ...
  function replaceNumericDefinedNames(
    expression: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    while (i < expression.length) {
      ...
      const scalar = resolveDefinedNameScalarValue?.(currentSheetName, token) || null;
      if (scalar != null) {
        const numeric = Number(scalar);
        if (!Number.isNaN(numeric)) {
  ...
  function replaceEmbeddedNumericFunctions(
    expression: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    while (changed) {
      ...
      current = current.replace(/[A-Z][A-Z0-9]*\([^()]*\)/gi, (segment) => {
        const resolved = tryResolveNumericFunction(
          segment,
          currentSheetName,
          resolveCellValue,
          resolveRangeValues,
  ...
  function resolveScalarFormulaValue(
    expression: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    if (scopedDefinedNameRef) {
      const scopedValue = resolveDefinedNameScalarValue?.(scopedDefinedNameRef.sheetName, scopedDefinedNameRef.name) || null;
      if (scopedValue != null) {
        return scopedValue;
      }
    }
    const definedNameValue = resolveDefinedNameScalarValue?.(currentSheetName, trimmed) || null;
  ...
  function tryResolveAggregateFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    resolveRangeEntries?: (sheetName: string, rangeText: string) => { rawValues: string[]; numericValues: number[] }
  ): string | null {
    if (!resolveRangeValues || !resolveRangeEntries) return null;
  ...
  function tryResolveConditionalAggregateFunction(
    ...
    if (sumifsCall) {
      ...
      if (sumCells.length === 0) return null;
      const rangeCriteriaPairs = [];
      for (let index = 1; index < args.length; index += 2) {
        const rangeRef = parseQualifiedRangeReference(args[index], currentSheetName);
        const criteria = resolveScalarFormulaValue(args[index + 1], currentSheetName, resolveCellValue);
        if (!rangeRef || criteria == null) return null;
        const cells = collectRangeCells(rangeRef, resolveCellValue);
        if (cells.length !== sumCells.length) return null;
        rangeCriteriaPairs.push({ cells, criteria });
      }
      let sum = 0;
    ...
    if (countifsCall) {
      ...
      if (rangeCriteriaPairs.some((entry) => entry.cells.length !== length)) return null;
      let count = 0;
    ...
    if (criteriaCells.length === 0) return null;
    ...
    if (!sumRange) return null;
    const sumCells = collectRangeCells(sumRange, resolveCellValue);
    if (sumCells.length !== criteriaCells.length) return null;

    let count = 0;
    let sum = 0;
    for (let i = 0; i < criteriaCells.length; i += 1) {
      if (!matchesCountIfCriteria(criteriaCells[i], criteria)) continue;
      count += 1;
      const numeric = Number(sumCells[i]);
      if (!Number.isNaN(numeric)) {
        sum += numeric;
  ...
  function tryResolveNumericFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    if (fnName === "DATE") {
      if (args.length !== 3) return null;
      const year = resolveNumericFormulaArgument(
        args[0],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const month = resolveNumericFormulaArgument(
        args[1],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const day = resolveNumericFormulaArgument(
        args[2],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
    ...
    if (fnName === "EOMONTH") {
      if (args.length !== 2) return null;
      const startValue = resolveScalarFormulaValue(
        args[0],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const monthOffset = resolveNumericFormulaArgument(
        args[1],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      if (startValue == null || monthOffset == null) return null;
    ...
    if (fnName === "INT") {
      if (args.length !== 1) return null;
      const value = resolveNumericFormulaArgument(
        args[0],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
    ...
    const digits = resolveNumericFormulaArgument(
      args[1],
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
  ...
  function tryResolveStringFunction(
    normalizedFormula: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    if (fnName === "MID") {
      ...
      const start = resolveNumericFormulaArgument(
        args[1],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const count = resolveNumericFormulaArgument(
        args[2],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      if (source == null || start == null || count == null) return null;
    ...
    if (fnName === "SUBSTITUTE") {
      ...
      const oldText = resolveScalarFormulaValue(
        args[1],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const newText = resolveScalarFormulaValue(
        args[2],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      if (source == null || oldText == null || newText == null || oldText === "") return null;
      if (args.length === 3) {
        return source.split(oldText).join(newText);
      }
      const instanceNum = resolveNumericFormulaArgument(
        args[3],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      if (instanceNum == null) return null;
    ...
    if (fnName === "REPLACE") {
      ...
      const replacement = resolveScalarFormulaValue(
        args[3],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      if (source == null || start == null || count == null || replacement == null) return null;
      const startIndex = Math.max(0, Math.trunc(start) - 1);
      const length = Math.max(0, Math.trunc(count));
    ...
    if (fnName === "REPT") {
      ...
      const countValue = resolveScalarFormulaValue(
        args[1],
        currentSheetName,
        resolveCellValue,
        resolveRangeValues,
      ...
      const normalizedCount = countValue == null
        ? (() => {
          const evaluatedCondition = evaluateFormulaCondition(
            args[1],
            currentSheetName,
            resolveCellValue,
            resolveRangeValues,
  ...
  function splitFormulaArguments(argText: string): string[] {
    const args: string[] = [];
    let current = "";
    let depth = 0;
    let inSingleQuote = false;
  ...
  function resolveAggregateArgument(
    ...
    resolveRangeValues: (sheetName: string, rangeText: string) => number[],
    resolveRangeEntries: (sheetName: string, rangeText: string) => { rawValues: string[]; numericValues: number[] }
  ): { numericValues: number[]; valueCount: number } | null {
  ...
  function resolveNumericFormulaArgument(
    expression: string,
    currentSheetName: string,
    resolveCellValue: (sheetName: string, address: string) => string,
    resolveRangeValues?: (sheetName: string, rangeText: string) => number[],
    ...
    const scalar = resolveScalarFormulaValue(
      expression,
      currentSheetName,
      resolveCellValue,
      resolveRangeValues,
  ...
  function collectRangeCells(
    rangeRef: { sheetName: string; start: string; end: string },
    resolveCellValue: (sheetName: string, address: string) => string
  ): string[] {
    const start = parseCellAddress(rangeRef.start);
    const end = parseCellAddress(rangeRef.end);
  ...
  function matchesCountIfCriteria(value: string, criteria: string): boolean {
  ...
  function parseQualifiedRangeReference(
    argText: string,
    currentSheetName: string
  ): { sheetName: string; start: string; end: string } | null {
    ...
    if (!qualifiedRangeMatch && !localRangeMatch) {
      ...
      if (scopedDefinedName) {
        const scopedRange = resolveDefinedNameRangeRef?.(scopedDefinedName.sheetName, scopedDefinedName.name) || null;
        if (scopedRange) {
          return scopedRange;
        }
      }
      const structuredRange = resolveStructuredRangeRef?.(currentSheetName, String(argText || "").trim()) || null;
      if (structuredRange) {
        return structuredRange;
      }
      const definedRange = resolveDefinedNameRangeRef?.(currentSheetName, String(argText || "").trim()) || null;
      if (definedRange) {
        return definedRange;
      }
      return null;
  ...
  function tokenizeArithmeticExpression(expression: string): string[] {
    const tokens: string[] = [];
    let index = 0;
  ...
  function isNarrativeListCandidate(item: NarrativeBlock["items"][number]): boolean {
    ...
    if (item.cellValues.length >= 2) {
      ...
      if (marker.length <= 6) return true;
    }
  ...
  function formatNarrativeListItem(item: NarrativeBlock["items"][number]): string {
  ...
  function matrixFromCandidate(sheet: ParsedSheet, candidate: TableCandidate, options: MarkdownOptions): string[][] {
  ...
  function createWorkbookExportArchive(workbook: ParsedWorkbook, markdownFiles: MarkdownFile[]): Uint8Array {
