(() => {
  ...
  type FormulaLegacyDeps = {
    ...
    getDefinedNameRangeRef: () => ((sheetName: string, name: string) => { sheetName: string; start: string; end: string } | null) | null;
    getStructuredRangeRef: () => ((sheetName: string, text: string) => { sheetName: string; start: string; end: string } | null) | null;
    cellFormat: {
      formatTextFunctionValue: (value: string, formatText: string) => string | null;
      parseValueFunctionText: (source: string) => number | null;
      datePartsToExcelSerial: (yyyy: number, mm: number, dd: number) => number | null;
      parseDateLikeParts: (source: string) => { yyyy: string; mm: string; dd: string } | null;
    };
  };

  function createFormulaLegacyApi(deps: FormulaLegacyDeps) {
    function tryResolveFormulaExpressionLegacy(
      ...
      if (datePartFunctionResult != null) return datePartFunctionResult;
      ...
      if (predicateFunctionResult != null) return predicateFunctionResult;
      ...
      if (chooseFunctionResult != null) return chooseFunctionResult;
      ...
      if (textFunctionResult != null) return textFunctionResult;
      const lookupFunctionResult = tryResolveLookupFunction(normalized, currentSheetName, resolveCellValue);
      if (lookupFunctionResult != null) return lookupFunctionResult;
      ...
      if (stringFunctionResult != null) return stringFunctionResult;
      const conditionalAggregateResult = tryResolveConditionalAggregateFunction(normalized, currentSheetName, resolveCellValue);
      if (conditionalAggregateResult != null) return conditionalAggregateResult;
    ...
    function tryResolveLookupFunction(
      ...
      if (xlookupCall) {
        ...
        if (args.length >= 5) {
          ...
          if (matchMode == null || !["0", ""].includes(matchMode.trim())) return null;
        }
        if (args.length >= 6) {
          const searchMode = resolveScalarFormulaValue(args[5], currentSheetName, resolveCellValue);
          if (searchMode == null || !["1", ""].includes(searchMode.trim())) return null;
        }
      ...
      if (matchCall) {
        ...
        if (args.length === 3) {
          ...
          if (matchType == null || !["0", ""].includes(matchType.trim())) return null;
        }
      ...
      if (hlookupCall) {
        ...
        if (args.length === 4) {
          ...
          if (!(normalizedLookup === "FALSE" || normalizedLookup === "0" || normalizedLookup === "")) return null;
        }
        const start = deps.parseCellAddress(rangeRef.start);
        const end = deps.parseCellAddress(rangeRef.end);
        if (!start.row || !start.col || !end.row || !end.col) return null;
      ...
      if (lookupValue == null || !rangeRef || Number.isNaN(columnIndex) || columnIndex < 1) return null;
      if (args.length === 4) {
        const rangeLookup = resolveScalarFormulaValue(args[3], currentSheetName, resolveCellValue);
        if (rangeLookup == null) return null;
        const normalizedLookup = rangeLookup.trim().toUpperCase();
    ...
    function tryResolvePredicateFunction(
      ...
      if (fnName === "ISNA") {
        if (/^\s*VLOOKUP\(/i.test(args[0])) return value == null ? "TRUE" : "FALSE";
      ...
      if (fnName === "ISTEXT") {
        ...
        if (normalized === "" || normalized === "TRUE" || normalized === "FALSE") return "FALSE";
        return Number.isNaN(Number(value)) ? "TRUE" : "FALSE";
      }
      return null;
    }
    ...
    function evaluateFormulaCondition(
      ...
      if (logical != null) return logical === "TRUE";
      ...
      if (comparison != null) return comparison === "TRUE";
    ...
    function replaceNumericDefinedNames(expression: string, currentSheetName: string): string {
      let result = "";
      let i = 0;
      let inSingleQuote = false;
      let inDoubleQuote = false;
    ...
    function replaceEmbeddedNumericFunctions(
      ...
      while (changed) {
        ...
        current = current.replace(/[A-Z][A-Z0-9]*\([^()]*\)/gi, (segment) => {
          ...
          if (Number.isNaN(numericValue)) return segment;
          changed = true;
          return String(numericValue);
        });
      }
    ...
    function resolveScalarFormulaValue(
      ...
      if (definedNameValue != null) return definedNameValue;
    ...
    function tryResolveAggregateFunction(
      ...
      if (resolvedArgs.some((entry) => entry == null)) return null;
      ...
      if ((fnName !== "COUNTA" && values.length === 0) || valueCount === 0) return null;
      if (fnName === "SUM") return String(values.reduce((sum, value) => sum + value, 0));
      if (fnName === "AVERAGE") return String(values.reduce((sum, value) => sum + value, 0) / values.length);
      ...
      if (fnName === "COUNTA") return String(valueCount);
      return null;
    }
    ...
    function tryResolveConditionalAggregateFunction(
      ...
      if (averageifsCall) {
        ...
        for (let i = 0; i < averageCells.length; i += 1) {
          if (!rangeCriteriaPairs.every((entry) => matchesCountIfCriteria(entry.cells[i], entry.criteria))) continue;
      ...
      if (fnName === "COUNTIF") return String(count);
      if (fnName === "SUMIF") return String(sum);
      return count > 0 ? String(sum / count) : null;
    }
    ...
    function tryResolveNumericFunction(
      ...
      if (fnName === "ROUND") return String(Math.round(value * factor) / factor);
      if (fnName === "ROUNDUP") {
    ...
    function tryResolveStringFunction(
      ...
      if (fnName === "SUBSTITUTE") {
        ...
        if (args.length === 3) return source.split(oldText).join(newText);
      ...
      if (fnName === "REPT") {
        ...
        const normalizedCount = countValue == null
          ? (() => {
            ...
            if (evaluatedCondition == null) return null;
    ...
    function matchesCountIfCriteria(value: string, criteria: string): boolean {
      ...
      const numericComparable = !Number.isNaN(leftNum) && !Number.isNaN(rightNum);
      if (operator === "=") return numericComparable ? leftNum === rightNum : value === operandText;
      if (operator === "<>") return numericComparable ? leftNum !== rightNum : value !== operandText;
      if (!numericComparable) return false;
    ...
    function parseQualifiedRangeReference(
      ...
      if (!qualifiedRangeMatch && !localRangeMatch) {
        ...
        if (definedRange) return definedRange;
        return null;
      }
    ...
    function evaluateArithmeticExpression(expression: string): number {
      ...
      function parseFactor(): number {
        ...
        if (Number.isNaN(numericValue)) throw new Error("Invalid token");
        return numericValue;
      }
    ...
    function tokenizeArithmeticExpression(expression: string): string[] {
      ...
      while (index < expression.length) {
        ...
        if (!numberMatch) throw new Error("Invalid arithmetic expression");
