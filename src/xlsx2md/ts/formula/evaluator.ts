...
(function initXlsx2mdFormulaEvaluator(global: typeof globalThis) {
  ...
  interface EvaluateFormulaAstContext {
    resolveCell?: (ref: string, sheet: string | null) => unknown;
  ...
  function evaluateFormulaAst(ast: FormulaAstNode, context: EvaluateFormulaAstContext = {}): unknown {
    switch (ast.type) {
      case "number":
        return ast.value;
      case "string":
      ...
      case "binary_op":
        ...
        return evaluateBinaryOp(
          ...
          evaluateFormulaAst(ast.right, context)
        );
      case "function_call":
        return evaluateFunctionCall(ast.name, ast.args, context);
      default:
        throw new Error(`Unsupported AST node: ${(ast as any).type}`);
    }
  }

  function evaluateRangeAst(ast: Extract<FormulaAstNode, { type: "range"; }>, context: EvaluateFormulaAstContext) {
  ...
  function evaluatePostfixOp(operator: string, operand: unknown) {
    if (operator === "%") {
  ...
  function evaluateBinaryOp(operator: string, left: unknown, right: unknown) {
    switch (operator) {
      case "+":
  ...
  function evaluateIf(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateIfError(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateAnd(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args.every((arg) => toBoolean(evaluateFormulaAst(arg, context)));
  }

  function evaluateOr(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args.some((arg) => toBoolean(evaluateFormulaAst(arg, context)));
  }

  function evaluateNot(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return !toBoolean(evaluateFormulaAst(args[0], context));
  }

  function evaluateDate(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateValue(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateInt(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return Math.floor(toNumber(evaluateFormulaAst(args[0], context)));
  }

  function evaluateAbs(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return Math.abs(toNumber(evaluateFormulaAst(args[0], context)));
  }

  function evaluateSum(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args
      .flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context)))
      .reduce((sum, value) => sum + toNumber(value), 0);
  }

  function evaluateSumProduct(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateRept(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateSubstitute(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
  ...
  function evaluateMatch(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateIndex(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateVLookup(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const lookupValue = evaluateFormulaAst(args[0], context);
  ...
  function evaluateHLookup(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const lookupValue = evaluateFormulaAst(args[0], context);
    const table = normalizeToMatrix(evaluateFormulaAst(args[1], context));
  ...
  function evaluateXLookup(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const lookupValue = evaluateFormulaAst(args[0], context);
    const lookupArray = flattenValues(evaluateFormulaAst(args[1], context));
  ...
  function evaluateWeekday(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateDateValue(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateLen(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return toText(evaluateFormulaAst(args[0], context)).length;
  }

  function evaluateLower(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return toText(evaluateFormulaAst(args[0], context)).toLowerCase();
  }

  function evaluateFind(args: FormulaAstNode[], context: EvaluateFormulaAstContext, ignoreCase: boolean) {
  ...
  function evaluateLeft(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
    const count = args[1] ? Math.max(0, Math.floor(toNumber(evaluateFormulaAst(args[1], context)))) : 1;
    return text.slice(0, count);
  }

  function evaluateRight(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
    const count = args[1] ? Math.max(0, Math.floor(toNumber(evaluateFormulaAst(args[1], context)))) : 1;
    return count === 0 ? "" : text.slice(-count);
  }

  function evaluateMid(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
  ...
  function evaluateTrim(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
    return text.trim().replace(/\s+/g, " ");
  }

  function evaluateReplace(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const text = toText(evaluateFormulaAst(args[0], context));
    const start = Math.max(1, Math.floor(toNumber(evaluateFormulaAst(args[1], context))));
    const count = Math.max(0, Math.floor(toNumber(evaluateFormulaAst(args[2], context))));
  ...
  function evaluateDay(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const serial = coerceDateSerial(evaluateFormulaAst(args[0], context));
    return excelSerialToDateParts(serial).day;
  }

  function evaluateMonth(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const serial = coerceDateSerial(evaluateFormulaAst(args[0], context));
    return excelSerialToDateParts(serial).month;
  }

  function evaluateYear(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const serial = coerceDateSerial(evaluateFormulaAst(args[0], context));
    return excelSerialToDateParts(serial).year;
  }

  function evaluateSubtotal(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateUpper(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return toText(evaluateFormulaAst(args[0], context)).toUpperCase();
  }

  function evaluateConcatenate(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args.map((arg) => toText(evaluateFormulaAst(arg, context))).join("");
  }

  function evaluateIsBlank(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const value = evaluateFormulaAst(args[0], context);
    return value === null || value === undefined || value === "";
  }

  function evaluateIsNumber(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const value = evaluateFormulaAst(args[0], context);
  ...
  function evaluateIsText(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const value = evaluateFormulaAst(args[0], context);
    return typeof value === "string";
  }

  function evaluateIsError(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return isFormulaError(evaluateFormulaAst(args[0], context));
  }

  function evaluateIsNa(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateMin(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const values = args.flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context))).map((value) => toNumber(value));
    return values.length ? Math.min(...values) : 0;
  }

  function evaluateMax(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const values = args.flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context))).map((value) => toNumber(value));
    return values.length ? Math.max(...values) : 0;
  }

  function evaluateAverage(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const values = args.flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context))).map((value) => toNumber(value));
    if (!values.length) {
      return "#DIV/0!";
    }
  ...
  function evaluateColumn(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    if (!args.length) {
  ...
  function evaluateRow(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    if (!args.length) {
      if (context.currentCellRef) {
  ...
  function evaluateEDate(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateEoMonth(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const startSerial = coerceDateSerial(evaluateFormulaAst(args[0], context));
    const months = Math.trunc(toNumber(evaluateFormulaAst(args[1], context)));
    const parts = excelSerialToDateParts(startSerial);
  ...
  function evaluateCountIf(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateCount(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args
      .flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context)))
  ...
  function evaluateCountA(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    return args
      .flatMap((arg) => flattenValues(evaluateFormulaAst(arg, context)))
  ...
  function evaluateSumIf(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateCountIfs(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const criteriaPairs: Array<{ values: unknown[]; criteria: string }> = [];
  ...
  function evaluateSumIfs(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function evaluateAverageIf(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
    const criteriaValues = flattenValues(evaluateFormulaAst(args[0], context));
    const criteria = toText(evaluateFormulaAst(args[1], context));
  ...
  function evaluateAverageIfs(args: FormulaAstNode[], context: EvaluateFormulaAstContext) {
  ...
  function excelSerialFromDate(year: number, month: number, day: number) {
  ...
  function coerceDateSerial(value: unknown) {
    if (typeof value === "number") {
      return value;
    }
  ...
  function columnNumberFromRef(ref: string) {
  ...
  function normalizeToMatrix(value: unknown): unknown[][] {
    if (!Array.isArray(value)) {
      return [[value]];
    }
    if (value.length > 0 && Array.isArray(value[0])) {
  ...
  function matchesCriteria(value: unknown, criteria: string) {
  ...
  function isCountableNumber(value: unknown) {
    if (typeof value === "number") {
      return true;
    }
    if (typeof value === "string") {
  ...
  function looselyEquals(left: unknown, right: unknown) {
